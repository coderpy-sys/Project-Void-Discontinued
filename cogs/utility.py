...
class Tools(commands.Cog):
    ...
    user = discord.SlashCommandGroup(name="user", description="User commands")

    @user.command(name="info", description="Fetch a Discord user and display their details.")
    async def user_info(self, ctx, user_id: str):
        ...
        badges = ", ".join([badge.replace("_", " ").title() for badge in user_data.get("badges", [])]) if user_data.get("badges") else "None"
        try:
            created_at_dt = datetime.fromisoformat(created_at.replace("Z", "+00:00"))
            created_at_str = created_at_dt.strftime("%B %d, %Y at %I:%M %p")
        except ValueError:
