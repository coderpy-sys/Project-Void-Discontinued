...
async def on_message(message):
    ...
    async with aiosqlite.connect("./db/database.db") as db:
        async with db.execute("SELECT xp, level, last_message_time FROM users WHERE id = ?", (message.author.id,)) as cursor:
            ...
            if row:
                ...
                if last_message_time is None:
                    last_message_time = 0  # Default to 0 if last_message_time is not set
